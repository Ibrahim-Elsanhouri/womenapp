...
global $connection;
$cos_id = $_GET['cos_id'];
$get_cos_products = "select * from cos_products where cos_id='$cos_id'";
$run_get_cos_products = mysqli_query($connection , $get_cos_products);
while ($cos_products_row = mysqli_fetch_array($run_get_cos_products)){
$cos_products = $cos_products_row['cos_products'];
echo "<input type='checkbox' name='products[]' value='$cos_products' /> <img src='cos/images/$cos_products'  height='80' width='80'> <br/>";
}
?>
        </fieldset>
				...
				<br>
						...
						<input type="submit" name="cos_btn" value="احجز الان">
          </form>
		  <?php 
if (isset($_POST['cos_btn'])){	
	...
	foreach($_POST['products'] as $products){
		$insert_cos_order = "insert into cos_order (cos_id ,  mobile , products , time  ) values ('$cos_id','$mobile','$products','$date')";
		$run_insert_cos_order = mysqli_query($connection , $insert_cos_order);
		if ($run_insert_cos_order){
