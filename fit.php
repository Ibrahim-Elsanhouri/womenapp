		...
		<div class="container">
			<div class="contact-grids">
				<div class="col-md-7 contact-form">
					<form action="fit.php" method="post">
						<input type="email" class="email" name="Email" placeholder="Email" required="">
						<input type="text" placeholder="سؤالك و استفسارك" name="Message" required="">
						<input type="submit" name="fit" value="ارسل ">
          </form>
          <?php 
if (isset($_POST['fit'])){
  global $connection;
  ...
  $add_query = "insert into fit ( email , query ) values ('$email','$query')";
  $run_add_qury = mysqli_query($connection , $add_query);
  if ($run_add_qury){
	echo "<script>alert('تم ارسال استشارتك فيما يخص الرشاقة  و سوف يتم الرد من قبل اختصاصي التغذية  في غضون 24 ساعة شكرا لك ')</script>";
