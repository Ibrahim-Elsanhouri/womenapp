						...
						<div class="w3-header-top-right">
							<div class="agileinfo-social-grids">
								<ul>
									...
									<li><a href="index.php"><i class="fa fa-vk"></i></a></li>
								</ul>
							</div>
			...
			<div class="contact-grids">
				<div class="col-md-7 contact-form">
					<form action="" method="post">
						<input type="text" name="mobile" placeholder="ادخل رقم هاتفك" required="">
						<input type="date" name="date" id="">
						<input type="time" name="time" id="">
				<br><br><br>
						<fieldset>
                <legend>الخدمات التي ترغب بها </legend>
				<?php 
global $connection;
$saloon_id = $_GET['saloon_id'];
$get_saloon_products = "select * from saloon_products where saloon_id='$saloon_id'";
$run_get_saloon_products = mysqli_query($connection , $get_saloon_products);
while ($saloon_products_row = mysqli_fetch_array($run_get_saloon_products)){
$saloon_products = $saloon_products_row['saloon_products'];
echo "<input type='checkbox' name='products[]' value='$saloon_products' />$saloon_products <br>";
}
?>
        </fieldset>
						...
						<input type="submit" name="sorder_btn" value="احجز الان">
          </form>
		  <?php 
if (isset($_POST['sorder_btn'])){	
	$mobile = $_POST['mobile'];
	$time = $_POST['time'];
	$date = $_POST['date'];
	foreach($_POST['products'] as $products){
		$insert_saloon_order = "insert into saloon_order (saloon_id , mobile , time , date , products ) values ('$saloon_id','$mobile','$time','$date','.$products.')";
		$run_insert_saloon_order = mysqli_query($connection , $insert_saloon_order);
	echo  "<script>window.alert('تم حجز موعد و ارسال طلباتك')</script>";	
