			...
			<div class="contact-grids">
				<div class="col-md-7 contact-form">
					<form action="" method="post" >
						<input type="text" name="mobile" placeholder="ادخل رقم هاتفك" required=""><br><br><br>
						<fieldset>
                <legend>التصاميم </legend>
				<?php 
global $connection;
$arrange_id = $_GET['arrange_id'];
$get_arrange_products = "select * from arrange_products where arrange_id='$arrange_id'";
$run_get_arrange_products = mysqli_query($connection , $get_arrange_products);
while ($arrange_products_row = mysqli_fetch_array($run_get_arrange_products)){
$arrange_products = $arrange_products_row['image'];
echo "<input type='checkbox' name='products[]' value='$arrange_products' /> <img src='designers/images/$arrange_products'  height='80' width='80'> <br/>";
}
?>
        </fieldset>
				...
				<br>
						...
						<input type="submit" name="arrange_btn" value="احجز الان">
          </form>
		  <?php 
if (isset($_POST['arrange_btn'])){	
	...
	foreach($_POST['products'] as $products){
		$insert_arrange_order = "insert into arrange_order (arrange_id ,  mobile , event , time  ) values ('$arrange_id','$mobile','$products','$date')";
		$run_insert_arrange_order = mysqli_query($connection , $insert_arrange_order);
		if ($run_insert_arrange_order){
	echo  "<script>window.alert('تم ارسال نوع التصميم و تاريخ التسليم المرغوب  ')</script>";	
