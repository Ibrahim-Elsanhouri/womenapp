			...
			<div class="contact-grids">
				<div class="col-md-7 contact-form">
					<form action="beauty.php" method="post">
						...
						<input type="text"  name="Message" placeholder="سؤالك و استفسارك" required="">

						<input type="submit" name="btn" value="ارسل ">
          </form>
          <?php 
if (isset($_POST['btn'])){
  global $connection;
  ...
  $add_query = "insert into beauty ( email , query ) values ('$email','$query')";
  $run_add_qury = mysqli_query($connection , $add_query);
  if ($run_add_qury){
	echo "<script>alert('تم ارسال استشارتك فيما يخص جمالك  و سوف يتم الرد من قبل اختصاصي التجميل   في غضون 24 ساعة شكرا لك ')</script>";
